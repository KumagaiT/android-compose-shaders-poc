...
// Hash "Hash Without Sine" - Extremely stable to avoid grid artifacts
float hash(float2 p) {
...
float fbm(float2 p) {
    float v = 0.0;
    float a = 0.5;
    // Using a small offset and non-integer multipliers to break patterns
    for (int i = 0; i < 5; ++i) {
...
half4 main(float2 fragCoord) {
    ...
    // Smoke intensity with larger initial scale
    float intensity = fbm(p * 2.5);
    
    // Refined contrast
    intensity = smoothstep(0.1, 0.9, intensity);

    // Circular vignette corrected by aspect ratio
